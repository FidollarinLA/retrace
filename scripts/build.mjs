...
const moon = process.env.MOON_BIN || (existsSync(join(homedir(), '.moon/bin/moon')) ? join(homedir(), '.moon/bin/moon') : 'moon');
for (const args of [['check','--target','js','--deny-warn'],['build','--target','js','--release']]) {
