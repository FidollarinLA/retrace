...
export function checkDraft(csv,plan,draft){
  const result=JSON.parse(verify(csv,JSON.stringify(plan),typeof draft==='string'?draft:JSON.stringify(draft)));
  if(!result.ok) throw new Error(result.error);
  return result;
}
...
export async function createBundle(csv,plan){
  ...
  return {format:'retrace-bundle',schema_version:1,engine_version:ENGINE_VERSION,csv,plan,report:compute(csv,plan),fingerprint:await fingerprint(csv,plan)};
}
export async function replay(bundle){
  if(!bundle || bundle.format!=='retrace-bundle' || bundle.schema_version!==1 || bundle.engine_version!==ENGINE_VERSION || typeof bundle.csv!=='string' || !bundle.plan || !bundle.report || typeof bundle.fingerprint!=='string') throw new Error('不支持的报告包或引擎版本。需要 ReTrace 0.1.0 / schema 1。');
  const fresh=await createBundle(bundle.csv,bundle.plan);
  return {bundle:fresh,source_matches:fresh.fingerprint===bundle.fingerprint,report_matches:canonical(fresh.report)===canonical(bundle.report)};
}
export function modelPrompt(report){
  const evidence=report.evidence.map(({id,group,value})=>({evidence_id:id,group,value,unit:report.unit})).filter(e=>e.value!==null);
...
export function reportHTML(bundle){
  const r=bundle.report,e=escapeHTML;
