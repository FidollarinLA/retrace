...
For a ready-to-run static copy, download the [0.2.2 demo ZIP](https://github.com/FidollarinLA/retrace/releases/download/v0.2.2/retrace-demo-0.2.2.zip), extract it, run `python3 -m http.server 4173` in the extracted folder and open `http://localhost:4173`.
...
`compare` replays and verifies both bundles first. It refuses tampered reports, aligns groups only when the grouping definition is stable, and never assigns a numerical change to one setting when several changed together.
